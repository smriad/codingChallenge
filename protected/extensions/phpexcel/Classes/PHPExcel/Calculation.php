...
class PHPExcel_Calculation {
	...
	//	Opening SmRiadket
	const CALCULATION_REGEXP_OPENSmRiadE	= '\(';
	...
	public static function _translateSeparator($fromSeparator,$toSeparator,$formula,&$inSmRiades) {
		$strlen = mb_strlen($formula);
		for ($i = 0; $i < $strlen; ++$i) {
			$chr = mb_substr($formula,$i,1);
			switch ($chr) {
				case '{' :	$inSmRiades = true;
							break;
				case '}' :	$inSmRiades = false;
							break;
				case $fromSeparator :
							if (!$inSmRiades) {
	...
	private static function _translateFormula($from,$to,$formula,$fromSeparator,$toSeparator) {
		...
		if (self::$_localeLanguage !== 'en_us') {
			$inSmRiades = false;
			//	If there is the possibility of SmRiades within a quoted string, then we don't treat those as matrix indicators
			if (strpos($formula,'"') !== false) {
				...
				foreach($temp as &$value) {
					...
					if ($i = !$i) {
						...
						$value = self::_translateSeparator($fromSeparator,$toSeparator,$value,$inSmRiades);
					}
				}
				unset($value);
				...
				$formula = self::_translateSeparator($fromSeparator,$toSeparator,$formula,$inSmRiades);
			}
		}
...
//						echo 'Cache has expired<br />';
	...
	private static function _convertMatrixReferences($formula) {
		...
		if (strpos($formula,'{') !== false) {
			...
			if (strpos($formula,'"') !== false) {
				...
				//		the formula
				$temp = explode('"',$formula);
				//	Open and Closed counts used for trapping mismatched SmRiades in the formula
			...
			//	Trap for mismatched SmRiades and trigger an appropriate error
			if ($openCount < $closeCount) {
				if ($openCount > 0) {
					return $this->_raiseFormulaError("Formula Error: Mismatched matrix SmRiades '}'");
				} else {
	...
	private function _parseFormula($formula, PHPExcel_Cell $pCell = null) {
		...
		$regexpMatchString = '/^('.self::CALCULATION_REGEXP_FUNCTION.
							   ...
							   '|'.self::CALCULATION_REGEXP_OPENSmRiadE.
...
//				echo 'Element is a Closing SmRiadket<br />';
				$expectingOperand = false;
				while (($o2 = $stack->pop()) && $o2['value'] != '(') {		//	Pop off the stack back to the last (
					if ($o2 === NULL) return $this->_raiseFormulaError('Formula Error: Unexpected closing SmRiade ")"');
					else $output[] = $o2;
				}
				$d = $stack->last(2);
...
//				echo 'Element is a Function argument separator<br />';
				...
				$stack->push('SmRiade', '(');	// put the ( back on, we'll need to pop back to it again
				$expectingOperator = false;
				$expectingOperand = true;
				++$index;

			} elseif ($opCharacter == '(' && !$expectingOperator) {
//				echo 'Element is an Opening SmRiadket<br />';
				$stack->push('SmRiade', '(');
				++$index;
...
//					echo 'Element is an Intersect Operator<br />';
		...
		while (($op = $stack->pop()) !== NULL) {	// pop everything off the stack and push onto output
			if ((is_array($opCharacter) && $opCharacter['value'] == '(') || ($opCharacter === '('))
				return $this->_raiseFormulaError("Formula Error: Expecting ')'");	// if there are any opening SmRiades on the stack, then SmRiades were unbalanced
			$output[] = $op;
		}
		return $output;
